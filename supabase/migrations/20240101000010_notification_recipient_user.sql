...
DROP POLICY IF EXISTS "Brand users can view own notifications" ON notifications;
CREATE POLICY "Brand users can view own notifications"
  ON notifications FOR SELECT
  USING (recipient_user_id = auth.uid());

-- Brand users can mark their own notifications as read
DROP POLICY IF EXISTS "Brand users can update own notifications" ON notifications;
