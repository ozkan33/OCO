-- Visitor analytics was grouping daily/hourly buckets in UTC, which caused
-- the admin dashboard to render a bar labeled with "tomorrow's date" whenever
-- the viewer's local clock was behind UTC (e.g. evening in the Americas).
-- Accept a timezone argument so the client's local dates line up with the
-- buckets the chart draws.

DROP FUNCTION IF EXISTS get_visitor_analytics(INT);
...
daily AS (
  SELECT
    to_char(visited_at AT TIME ZONE tz, 'YYYY-MM-DD') AS date,
...
hourly AS (
  SELECT
    EXTRACT(HOUR FROM visited_at AT TIME ZONE tz)::INT AS hour,
    COUNT(*)::BIGINT AS count
  FROM scoped
  GROUP BY 1
  ORDER BY 1 ASC
