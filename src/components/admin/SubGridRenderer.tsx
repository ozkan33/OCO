...
export default function SubGridRenderer({ parentId }: { parentId: string | number | undefined }) {
  ...
  const calculateColumnWidth = (col: MyColumn) => {
    ...
    const contentLengths = grid.rows.map((row: Row) => String(row[col.key] || '').length);
  ...
  const subEditableColumns: MyColumn[] = grid.columns.map((col: MyColumn, idx: number) => ({
    ...
    renderCell: (props: { row: Row }) => {
      ...
      return (
        <div className="flex items-center px-2 py-1" style={{ fontSize: '13px', minHeight: '24px', backgroundColor: 'transparent' }} title={isTruncated ? String(cellValue) : undefined}>
    ...
    renderEditCell: ({ row, column, onRowChange }: RenderEditCellProps<Row>) => (
      <input defaultValue={row[column.key] !== undefined ? String(row[column.key]) : ''} onChange={e => { if (e.target.value.length <= 20) onRowChange({ ...row, [column.key]: e.target.value }); }}
        className="w-full h-full px-2 py-1 border border-blue-300 rounded bg-white" autoFocus style={{ fontSize: '13px', height: '24px', minHeight: '24px' }} maxLength={20} />
    ),
  }));
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between gap-3 px-4 py-3 bg-slate-50 border-b border-slate-200 shrink-0">
        ...
        <button onClick={() => setExpandedRowId(null)} className="text-slate-400 hover:text-slate-600 text-xl font-bold shrink-0 w-8 h-8 flex items-center justify-center rounded-lg hover:bg-slate-100 transition-colors" aria-label="Close">&times;</button>
      </div>
